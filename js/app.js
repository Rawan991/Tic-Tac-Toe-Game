//returns a list of the document's elements
...
// array to push the value of x and o 
...
function assignValues(currentPlayer, location){
    let row = location.row - 1; // shift to match the array (arrays start at 0 not 1)
    let col = location.col - 1;
    let choice = currentPlayer;
...
function validateWinning(){
    ...
    _.forEach((row, index, array)=>{
        ...
        console.log(rowVals)
        const win =  rowVals.every( (val, i, arr) => val === arr[0] &&  val != ' ');
        console.log('win ? ', win)
